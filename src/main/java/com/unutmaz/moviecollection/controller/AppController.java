...
public class AppController {
	...
	public String confirmCreation(@ModelAttribute MovieFormWrapper mfw, Model model) {
		...
		String castRef = mfw.getCastRef();
		if (castRef != null && !castRef.isEmpty()) {
			for (String elem : castRef.split(",")) {
				try {
					Long actorId = Long.parseLong(elem.trim());
					Actor actor = appService.findActor(actorId);
					if (actor != null) {
						cast.add(actor);
					} else {
					...
					// Handle the case where elem is not a valid Long
